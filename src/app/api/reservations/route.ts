...
export async function GET(req: NextRequest) {
  ...
  const result = groups.map(([key, g]) => {
    ...
    return {
      ...
      slobodnih: g._vehicles.filter((v: any) => {
        if (occupiedPlates.has(v.license_plate)) return false
        // Provjeri regiju — vozilo mora biti u regiji lokacije preuzimanja
        if (pickupLocation) {
          const loc = pickupLocation.toLowerCase()
          const vLok = v.lokacija || ''
          const isCG = loc.includes('podgorica') || loc.includes('tivat') || loc.includes('bar') || loc.includes('budva') || loc.includes('kotor') || loc.includes('herceg') || loc.includes('ulcinj') || loc.includes('cetinje')
          const isBiH = loc.includes('sarajevo') || loc.includes('mostar') || loc.includes('banja')
          const isSrb = loc.includes('beograd') || loc.includes('novi sad')
          const isAlb = loc.includes('tirana') || loc.includes('durres')
          if (isCG && vLok !== 'CRNA GORA') return false
          if (isBiH && vLok !== 'BiH') return false
          if (isSrb && vLok !== 'SRBIJA') return false
          if (isAlb && vLok !== 'ALBANIJA') return false
        }
        return true
      }).length,
      lokacija: g.lokacija,
