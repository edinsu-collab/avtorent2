  ...
  try {
    ...
    try {
      ...
      const ge = guestEmail({
        ...
        insurance: insurance || 'Osnovno (AO)',
        insuranceTotal: insuranceTotal || 0,
        borderCrossing: borderCrossing || '',
        flightNumber: flightNumber || '',
        hasSecondDriver: hasSecondDriver || false,
        driver2Name: driver2Name || '',
        guestNationality: guestNationality || '',
        days: Math.ceil((new Date(returnDate).getTime() - new Date(pickupDate).getTime()) / 86400000),
        pricePerDay: basePrice || 0,
        assignedVehicle: resolvedVehiclePlate || null,
        extras: (extras || []).map((e: any) => ({ name: e.name || e.id, price: e.total || 0 })),
      ...
      const ae = adminEmail({
        ...
        hasSecondDriver: hasSecondDriver || false,
        driver2Name: driver2Name || '',
        driver2License: driver2License || '',
        guestNationality: guestNationality || '',
        guestDob: guestDob || '',
        guestLicense: guestLicense || '',
        extras: (extras || []).map((e: any) => ({ name: e.name || e.id, price: e.total || 0 })),
        partnerName: partner?.name, commissionAmount,
